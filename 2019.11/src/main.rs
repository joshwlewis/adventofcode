...
fn main() {
    ...
    let intcode: Vec<isize> = strcode.split(',').filter_map(|i|
...
impl Panel {
    fn next(&self, dir: Dir) -> Panel {
        match dir {
            Dir('^') => Panel{x: self.x,   y: self.y+1},
            Dir('>') => Panel{x: self.x+1, y: self.y},
            Dir('v') => Panel{x: self.x,   y: self.y-1},
            Dir('<') => Panel{x: self.x-1, y: self.y},
